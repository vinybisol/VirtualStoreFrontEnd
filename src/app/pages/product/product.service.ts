...
export class ProductService {
  ...
  getAllProductAsync(): Observable<ProductModel[]> {
    const url = `${this.API}/product/getall`
    return this._http.get<ProductModel[]>(url).pipe(first())
  }
  getAllProductWithImagesAsync(): Observable<ProductModel[]> {
    return this._http.get<ProductModel[]>(`${this.API}/product/getall`).pipe(first())
  }
  getByIdAsync(key: string): Observable<ProductModel> {
    return this._http.get<ProductModel>(`${this.API}/product/getbyid/${key}`).pipe(first())
  }
  store(product: ProductModel): Observable<ProductModel> {
    const url = `${this.API}/product/add`
  ...
  storeImages(product: ProductModel, key: string): Observable<any> {
    ...
    return this._http.post(`${this.API}/Products/Images?productKey=${key}`, formData)
  }
